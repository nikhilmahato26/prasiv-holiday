                    ...
                    {(room.amenities || []).length > 0 && (
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                        {(room.amenities || []).map((a, j) => {
                          ...
                          return <span key={j} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12.5, color: '#374151', background: '#F3EFE1', borderRadius: 999, padding: '4px 10px' }}><Icon size={13} /> {a}</span>
                        })}
                      </div>
                    )}
