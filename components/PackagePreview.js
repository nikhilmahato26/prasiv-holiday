  ...
  return (
    <div style={{ fontSize: 14, color: '#111' }}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {pkg.hotels && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: '#F3EFE1', borderRadius: 999, fontSize: 12, color: '#6b7280', marginTop: 10 }}>
          ...
          {(pkg.amenities || []).length > 0 && (
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {(pkg.amenities || []).map((a, i) => <span key={i} style={{ fontSize: 12, color: '#374151', background: '#F3EFE1', borderRadius: 999, padding: '3px 10px' }}>{a}</span>)}
              </div>
            </div>
          )}
