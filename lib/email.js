...
const BRAND_GRADIENT = 'linear-gradient(135deg,#013893,#0252D8)'
const ACCENT = '#013893'
...
export async function sendOtpEmail(to, otp) {
  await sendMail({
    ...
    html: shell(`
      ...
      <div style="background:#E6F0FF;border-radius:12px;padding:20px;text-align:center;margin-bottom:24px">
...
export async function sendEnquiryEmail({ name, phone, email, message, package_title, destination, type }) {
  ...
  const html = shell(`
    <div style="display:inline-block;background:#E6F0FF;color:${ACCENT};font-size:12px;font-weight:700;padding:4px 12px;border-radius:999px;margin-bottom:12px">${esc(typeLabel)} Enquiry</div>
