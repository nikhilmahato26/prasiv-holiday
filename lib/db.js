...
export async function addCampaign(c) {
  ...
  await initCampaignsTable()
  const { rows } = await pool.query(
    'INSERT INTO campaigns (slug, name, title, description, image_url, offer_price, destination, package_details, other_information) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
    [c.slug, c.name, c.title, c.description, c.image_url, c.offer_price, c.destination, c.package_details, c.other_information]
  )
  return rows[0]
}
...
export async function updateCampaign(id, c) {
  const pool = getPool()
  const { rows } = await pool.query(
    'UPDATE campaigns SET slug=$1, name=$2, title=$3, description=$4, image_url=$5, offer_price=$6, destination=$7, package_details=$8, other_information=$9, updated_at=NOW() WHERE id=$10 RETURNING *',
    [c.slug, c.name, c.title, c.description, c.image_url, c.offer_price, c.destination, c.package_details, c.other_information, id]
  )
  return rows[0]
}
